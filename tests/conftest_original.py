...
@pytest.fixture(scope="session")
async def test_redis_client(redis_container: RedisContainer) -> AsyncGenerator[RedisClient, None]:
...
async def cleanup_database(db_session: AsyncSession):
    ...
    await db_session.execute("DELETE FROM role_scopes")
    await db_session.execute("DELETE FROM service_client_scopes")
    await db_session.execute("DELETE FROM users")
    await db_session.execute("DELETE FROM roles")
    await db_session.execute("DELETE FROM scopes")
    await db_session.execute("DELETE FROM service_clients")
    await db_session.commit()
